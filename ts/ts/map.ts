...
export const fallback =
  <V>(escapeIf: (v: V) => boolean) =>
  (v: V, defaultValue: V): V =>
...
export const mapUnionsToAverage =
  <K,V>(maps: Map<K,number>[])=>
    map_map(mapUnions(maps)(0, (s,d)=>s+d))((v, ) => ifdo(maps.length)(d => d != 0, d => v/d))
...
export const map_map = <K,V>(m: Map<K,V>) => <T>(mapf: (v: V, m: Map<K,V>) => T) =>
  new Map(
    [...m.entries()].map(([k,v])=> [k, mapf(v, m)])
  )