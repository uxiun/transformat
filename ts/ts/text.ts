...
export const isEmptyOrWhitespaces = (s: string) => !/\S/.test(s)
					...
					new Map(
						caseUndefined(s.index.get(seg))(
							v =>
								...
								// ),
								[...v.entries()].flatMap(([id, positions]) =>
									id == textid
										? [[id, [...positions, i]]]
										: [
												[id, positions],
...
export const ngramTargetFormat = (n: number, str: string): NgramTarget => ({
	n,
	m: groupByMapForList(list_entries(ngramSegmentify(n, str)))(([id, ngram]) => ngram)(
		([id, ngram]) => id
	),
})
...
const ngramHitPoint = (target: number[]) => (vs: NgramIndexData) =>
	new Map(
		[...vs.entries()].map(([id, positions]) => [
			id,
			// - target.length
			(+target.filter(d => positions.includes(d)).length * 1000 +
				(Math.min(positions.length, target.length) / Math.max(positions.length, target.length)) *
					100) /
...
export const concernLength =
	(scoreMap: IdScoreMap) => (targetLength: number, lengthmap: Map<number, number>) =>
		new Map(
			[...scoreMap.entries()].map(([id, score]) => [
				id,
		...
		return map_map(
			mapUnions(
				es.map(([ngram, positions], i, arr) =>
					ngramHitPoint(positions)(new Map([...(index.index.get(ngram) ?? [].entries())]))
				)
			)(0, (s, d, key) => s + d)
		)(v =>
			ifdo(es.length)(
